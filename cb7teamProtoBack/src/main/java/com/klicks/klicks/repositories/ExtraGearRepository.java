...
public interface ExtraGearRepository extends JpaRepository<ExtraGear, Integer>{
	...
	@Query(value="SELECT idextra_gear,price,description,piclink,name FROM extra_gear,session_gear WHERE idextra_gear = fk_gear_id AND fk_session_id = ?1",nativeQuery = true)
